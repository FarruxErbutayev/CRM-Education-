<template>
    <div class="w-1/2 mx-auto p-6 border border-gray-300 rounded-lg bg-gray-50">
      <h2 class="text-2xl font-semibold mb-6 text-center">O'quvchi qo'shish</h2>
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="mb-4">
          <label for="lastname" class="block text-sm font-medium text-gray-700">Familiya:</label>
          <input v-model="form.lastname" type="text" id="lastname" placeholder="Enter Lastname" required
            class="w-full p-2 mt-1 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
  
        <div class="mb-4">
          <label for="birth" class="block text-sm font-medium text-gray-700">Tug'ilgan kuni:</label>
  ...
  const submitForm = async () => {
    ...
    try {
      const response = await fetch('http://localhost:4100/pupil/add', {
        method: 'POST',
        headers: {
      ...
      successMessage.value = `New pupil added with ID: ${data.id}`;
      form.value = { firstname: '', lastname: '', birth: '', course_name: '' };  // Formani tozalash
    } catch (error) {
      errorMessage.value = `Failed to add pupil: ${error.message}`;
    } finally {
